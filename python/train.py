...
class Trainer():
    ...
    def train(self, args):
        ...
        train_loader, val_loader, test_loader = load_data(args.tokens_in_batch, run_dir, src_bpe_model, tgt_bpe_model)
        ...
        steps = self.train_n_epochs(args, epochs, model, train_loader, val_loader, test_loader, src_bpe_model, tgt_bpe_model, criterion, optimizer, positional_encoding, summary_writer)
    ...
    def train_n_epochs(self, args, epochs, model, train_loader, val_loader, test_loader, src_bpe_model, tgt_bpe_model, criterion, optimizer, positional_encoding, summary_writer):
        step = 1
...
if __name__ == '__main__':
    ...
    if args.prune_mode == 'train-prune':
        ...
        model, _, _ = load_checkpoint_or_generate_new(args, os.path.join('runs', args.run_name), device=args.device)
        prune_model(model, args.prune_heads_amount, args.prune_heads_norm, args.prune_ffn_amount, args.prune_ffn_norm, args.prune_structured, args.prune_type)
    else:
        trainer.train(args)
