...
class TranslationTrainer(base_trainer.BaseTrainer):
    ...
    def train_epoch(self, model: megatransformer.MegaTransformer, epoch):
        ...
        for i, (input_ids, labels) in enumerate(self.train_loader):
            ...
            src_key_padding_mask = input_ids == self.args.padding_value
            tgt_key_padding_mask = (labels == self.args.padding_value).to(self.args.decoder_device)
    ...
    def viz_model(self, step, model, src, **kwargs):
        ...
        with torch.no_grad():
            with autocast():
                ...
                visualization_helper.viz_model(self.args.encoder_device, self.args.decoder_device, model, self.summary_writer.add_image, step, self.args.maxlen, src, src_tokens, tgt, tgt_tokens, padding_value=self.args.padding_value)
